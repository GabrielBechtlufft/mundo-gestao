...
const STATUS_CFG: Record<
   string,
   { bg: string; text: string; label: string; cor: string }
> = {
   CONTATO_SOLICITADO: {
      bg: "#FEF3C7",
      text: "#92400E",
...
const ETAPAS = [
   {
      key: "CONTATO_SOLICITADO",
      label: "Contato Solicitado",
      desc: "Comprador solicitou contato com a certificadora.",
   },
   {
      key: "EM_CONTATO",
      label: "Em Contato",
      desc: "Certificadora iniciou o atendimento via chat.",
   },
   {
      key: "PROPOSTA_ENVIADA",
      label: "Proposta Enviada",
      desc: "Certificadora enviou a proposta comercial.",
   },
   {
      key: "EM_NEGOCIACAO",
      label: "Em Negociação",
      desc: "As partes estão negociando os termos.",
   },
   {
      key: "PROPOSTA_FECHADA",
      label: "Proposta Fechada",
      desc: "Negociação concluída e pagamento arquivado.",
   },
];
...
function Avatar({
   ...
   const cor = ["#6001D3", "#0891B2", "#059669", "#D97706"][
      nome.charCodeAt(0) % 4
   ];
   return (
      <div
...
function ModalHistorico({
   ...
   const timestampEtapa = (idx: number): Date | string | null => {
      ...
      if (idx === 4)
         return proposta.status === "PROPOSTA_FECHADA"
            ? proposta.updatedAt
            : null; // PROPOSTA_FECHADA
      return null;
   };
         ...
         >
            ...
            >
               <div>
                  ...
                  <p style={{ margin: 0, fontSize: "14px", color: "#6B7280" }}>
                     {proposta.Listagem?.titulo || proposta.servico}
                  </p>
               </div>
               <button
                  onClick={onClose}
            ...
            <div style={{ display: "flex", gap: "12px", marginBottom: "28px" }}>
               {[
                  {
                     label: "Comprador",
                     nome: proposta.Comprador?.name || proposta.solicitante,
                     email: proposta.Comprador?.email,
                     imagem: proposta.Comprador?.image,
                  },
                  {
                     label: "Certificadora",
                     nome:
                        proposta.Vendedor?.razaoSocial ||
                        proposta.Vendedor?.name ||
                        "—",
                     email: proposta.Vendedor?.email,
                     imagem: proposta.Vendedor?.image,
                  },
               ].map((p) => (
                  <div
                     key={p.label}
               ...
               {cancelada ? (
                  ...
                  >
                     ...
                     <div>
                        ...
                        >
                           Atualizada em {dataFormatada(proposta.updatedAt)}
                        </p>
                     </div>
                  </div>
               ) : (
                                    ...
                                    style={{
                                       ...
                                       background: concluida
                                          ? atual && etapaAtual < 3
                                             ...
                                             : "#DCFCE7"
                                          : "#F3F4F6",
                                       border: `2px solid ${concluida ? (atual && etapaAtual < 3 ? "#7B00D4" : "#16A34A") : "#E5E7EB"}`,
                                       display: "flex",
                                       alignItems: "center",
                                       justifyContent: "center",
                                       fontSize: "12px",
                              ...
                              >
                                 <p
                                    style={{
                                       margin: "3px 0 2px",
                                       fontSize: "14px",
                                       fontWeight: concluida ? 700 : 500,
                                       color: concluida ? "#111" : "#9CA3AF",
                                    }}
                                 >
                                    {etapa.label}
                                 </p>
            ...
            {proposta.documentoProposta && (
               ...
               >
                  ...
                  <div style={{ flex: 1 }}>
                     ...
                     >
                        Proposta enviada pela certificadora
                     </p>
                     <a
                        href={proposta.documentoProposta}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{ fontSize: "12px", color: "#6001D3" }}
                     >
                        Visualizar proposta ↗
                     </a>
                  </div>
            ...
            <div
               style={{
                  background: STATUS_CFG[proposta.status]?.bg ?? "#F3F4F6",
                  borderRadius: "12px",
                  padding: "12px 16px",
                  textAlign: "center",
               }}
            >
               <p
                  style={{
                     margin: "0 0 2px",
                     fontSize: "11px",
                     color: STATUS_CFG[proposta.status]?.text ?? "#374151",
                     fontWeight: 700,
                     textTransform: "uppercase",
                     letterSpacing: "0.5px",
                  }}
...
export default function PropostasPage() {
   ...
   const [modalProposta, setModalProposta] = useState<PropostaAdmin | null>(
      null,
   );
   const [filtro, setFiltro] = useState<
      "todas" | "ativas" | "concluidas" | "canceladas"
   >("todas");
   ...
   const lista = propostas.filter((p) => {
      if (
         filtro === "ativas" &&
         ["PROPOSTA_FECHADA", "CANCELADA"].includes(p.status)
      )
         return false;
      if (filtro === "concluidas" && p.status !== "PROPOSTA_FECHADA")
         return false;
      ...
      if (busca.trim()) {
         ...
         if (
            !compradorNome.includes(t) &&
            !vendedorNome.includes(t) &&
            !isoTipo.includes(t) &&
            !titulo.includes(t)
         )
            return false;
   ...
   const ativas = propostas.filter(
      (p) => !["PROPOSTA_FECHADA", "CANCELADA"].includes(p.status),
   ).length;
   const concluidas = propostas.filter(
      (p) => p.status === "PROPOSTA_FECHADA",
   ).length;
                     ...
                     >
                        ...
                        <span
                           style={{
                              fontSize: "13px",
                              fontWeight: 600,
                              color: filtro === c.key ? c.color : "#374151",
                           }}
                        >
                           {c.label}
                        </span>
               ...
               <div style={{ position: "relative" }}>
                  ...
                  <input
                     ...
                     style={{
                        ...
                        background: "#fff",
                        boxShadow: "0 2px 8px rgba(80,0,160,0.06)",
                        color: "#000",
                     }}
                     onFocus={(e) => {
                        e.currentTarget.style.borderColor = "#6001D3";
                     }}
                     onBlur={(e) => {
                  ...
                  {loading ? (
                     <div
                        style={{
                           ...
                           padding: "60px",
                           textAlign: "center",
                           boxShadow: "0 8px 32px rgba(80,0,160,0.1)",
                        }}
                     >
                     ...
                     lista.map((p) => {
                        const cfg =
                           STATUS_CFG[p.status] ??
                           STATUS_CFG.CONTATO_SOLICITADO;
                        const compradorNome =
                           p.Comprador?.name || p.solicitante;
                        const vendedorNome =
                           p.Vendedor?.razaoSocial || p.Vendedor?.name || "—";
                        ...
                        return (
                           <div
                              ...
                              style={{
                                 ...
                                 borderRadius: "20px",
                                 padding: "20px 24px",
                                 boxShadow: "0 4px 20px rgba(80,0,160,0.08)",
                                                   ...
                                                   style={{
                                                      ...
                                                      background: feita
                                                         ? atual
                                                            ...
                                                            : "#DCFCE7"
                                                         : "#F3F4F6",
                                                      border: `2px solid ${feita ? (atual ? "#7B00D4" : "#16A34A") : "#E5E7EB"}`,
                                                      display: "flex",
                                                      alignItems: "center",
                                                      justifyContent: "center",
                                                      fontSize: "10px",
