...
import { excluirListagemAdmin, getNormasAdmin, suspenderListagemAdmin, reativarListagemAdmin } from "@/app/actions/admin";
...
const statusStyle: Record<string, { label: string; color: string; background: string }> = {
  SUSPENSA_ADMIN: { label: "Suspensa pelo Admin", color: "#B91C1C", background: "#FEF2F2" },
...
export default function NormasAdminPage() {
  ...
  const carregar = () => getNormasAdmin().then((res) => { if (res.success) setNormas(res.normas as Norma[]); setLoading(false);
  });
