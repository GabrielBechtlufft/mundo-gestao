...
function certBadge(validade: string | null): {
   ...
   if (!validade)
      return { label: "Sem certificado", color: "#9CA3AF", bg: "#F3F4F6" };
   const dias = Math.ceil(
      (new Date(validade).getTime() - Date.now()) / 86400000,
   );
   if (dias < 0)
      return {
         label: `Exp. há ${Math.abs(dias)}d`,
         color: "#B91C1C",
         bg: "#FEF2F2",
      };
   if (dias <= 30)
      return { label: `Expira em ${dias}d`, color: "#92400E", bg: "#FFFBEB" };
...
export default function VendedoresPage() {
   ...
   const handleSuspender = async (id: string) => {
      if (
         !confirm(
            "Deseja suspender esta certificadora? Todas as normas serão pausadas.",
         )
      )
         return;
   ...
   const handleExcluir = async (id: string) => {
      if (!confirm("Excluir esta certificadora da operação? As normas vinculadas serão removidas da busca.")) return;
            ...
            >
               ...
               {certExpiradas.length > 0 && (
                  ...
                  >
                     ...
                     >
                        {certExpiradas.length} certificadora
                        {certExpiradas.length > 1 ? "s" : ""} com certificado
                        ISO expirado:{" "}
               ...
               >
                  <div>
                     ...
                     >
                        Certificadoras Cadastradas
                     </h2>
                     <span style={{ fontSize: "14px", color: "#888" }}>
                        {certificadoras.length} no total
                     </span>
                  </div>
                  <button
                           ...
                           >
                              ...
                              >
                                 ...
                                 >
                                    ...
                                    <button onClick={() => handleExcluir(v.id)} style={{ padding: "7px 16px", borderRadius: "8px", border: "1.5px solid #EF4444", color: "#EF4444", fontWeight: 600, fontSize: "13px", background: "transparent", cursor: "pointer" }}>Excluir</button>
                                 </div>
                              </div>
                           </div>
                        );
               ...
               >
                  ...
                  >
                     ...
                     <div>
                        ...
                        >
                           {detalhe.razaoSocial || detalhe.name}
                        </h2>
                        <p
                           style={{
                              margin: 0,
                  ...
                  {[
                     ...
                     { label: "Normas", value: `${detalhe._count.normas}` },
                     {
                        label: "Status",
                        value:
                           statusLabel[detalhe.statusVendedor] ||
                           detalhe.statusVendedor,
                     },
                  ].map(({ label, value }) => (
                     <div
                        key={label}
         ...
         {showModal && (
            ...
            >
               ...
               >
                  ...
                  >
                     Insira os dados da nova certificadora. A senha provisória
                     será <strong>senha@123</strong> e deverá ser alterada no
                     primeiro acesso.
                  </p>
