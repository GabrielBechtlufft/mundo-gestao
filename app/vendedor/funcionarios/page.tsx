...
type ModalTipo = "adicionar" | "editar" | "confirmarRemover" | null;
...
export default function FuncionariosPage() {
  ...
  const fechar = () => { setModal(null); setSelecionado(null); setErro(""); };
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px", marginTop: "8px", flexShrink: 0 }}>
        <div>
          ...
          <p style={{ color: "rgba(255,255,255,0.65)", fontSize: "14px", margin: 0 }}>
            Gerencie os funcionários vinculados à sua certificadora
          </p>
        </div>
        <button
            ...
            {loading ? (
              ...
              <div style={{ textAlign: "center", paddingTop: "60px", paddingBottom: "60px" }}>
                ...
                <p style={{ color: "#9CA3AF", fontSize: "14px", margin: "0 0 28px" }}>
                  Adicione os membros da sua equipe para que apareçam vinculados à sua certificadora.
                </p>
                <button
                  onClick={abrirAdicionar}
      ...
      {(modal === "adicionar" || modal === "editar") && (
        <Overlay onClose={fechar}>
          ...
          <p style={{ color: "#9CA3AF", fontSize: "13px", margin: "0 0 24px" }}>
            {modal === "adicionar"
              ? "Preencha os dados do funcionário que será vinculado à sua certificadora."
              : "Atualize as informações do funcionário."}
          </p>

          <CampoTexto label="Nome completo *" value={nome} onChange={setNome} placeholder="Ex: João da Silva" />
          <CampoTexto label="Cargo" value={cargo} onChange={setCargo} placeholder="Ex: Auditor Líder, Consultor ISO 9001..." />
          <CampoTexto label="E-mail" value={email} onChange={setEmail} placeholder="joao@empresa.com.br" type="email" />
...
function CartaoFuncionario({ funcionario: f, onEditar, onToggleAtivo, onRemover }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "15px", fontWeight: 700, color: "#111", marginBottom: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {f.nome}
        </div>
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          {!f.cargo && !f.email && (
            <span style={{ fontSize: "12px", color: "#D1D5DB", fontStyle: "italic" }}>Sem cargo ou e-mail</span>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
        ...
        <button
          onClick={onEditar}
          title="Editar"
          style={{ width: "36px", height: "36px", borderRadius: "10px", border: "1.5px solid #E5E7EB", background: "#F9FAFB", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", transition: "all 0.2s" }}
        >
          ✎
        </button>
        ...
        <button
          onClick={onRemover}
          title="Remover"
          style={{ width: "36px", height: "36px", borderRadius: "10px", border: "1.5px solid #FECACA", background: "#FEF2F2", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", color: "#EF4444", transition: "all 0.2s" }}
        >
          ✕
        </button>
      </div>
